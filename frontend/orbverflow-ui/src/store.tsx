...
const Ctx = createContext<{ state: AppState; actions: { toast: (m: string) => void } } | null>(null);
...
export function AppStoreProvider({ children }: { children: React.ReactNode }) {
  ...
  function toast(message: string) {
    setToastMsg(message);
    window.setTimeout(() => setToastMsg((cur) => (cur === message ? null : cur)), 1800);
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      //   if (Array.isArray(p?.playbooks) && p.playbooks.length) setPlaybooks(p.playbooks);
      // } catch {}
  ...
  useEffect(() => {
    const ws = connectWs((e: any) => {
      ...
      if (e.type === "fleet_snapshot") {
        ...
        if (!datasetMeta && v) {
          setDatasetMeta({ source_vendor: v, source_dataset_id: "UNKNOWN", mapping_version: null, files: [] });
        }
      }
      ...
      if (e.type === "playbook_approved") {
        ...
        setApprovedPlaybooks((prev) => {
          const rec = { id: pbId, state: "APPROVED", delivery_status: e.delivery_status ?? "SENT" };
  ...
  const state = useMemo<AppState>(
    ...
    [datasetMeta, scenario, fleet, latestIncident, playbooks, approvedPlaybooks, mission, audit]
  );

  return (
    <Ctx.Provider value={{ state, actions: { toast } }}>
